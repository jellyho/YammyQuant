...
| **Research** | Run a **backtest** (equity curve with a dashed **buy & hold** benchmark + an **underwater drawdown** chart + a **monthly-returns heatmap** + a price/signals overlay), **optimize** (grid → a 2-param **sensitivity heatmap**, or walk-forward bars), or a **multi-symbol portfolio backtest** on any ticker(s) + strategy — research without leaving the dashboard. |
...
Almost every component is manageable from here, not just viewable: it's a full
control surface over the same state the CLI drives.
