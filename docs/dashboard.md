...
| **Research** | Run a **backtest** (equity curve with a dashed **buy & hold** benchmark + an **underwater drawdown** chart + a **monthly-returns heatmap** + a price/signals overlay), **optimize** (grid / walk-forward), or a **multi-symbol portfolio backtest** on any ticker(s) + strategy — research without leaving the dashboard. |
